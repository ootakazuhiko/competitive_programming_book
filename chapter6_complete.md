...
![図6-5：リスト検索 vs 辞書検索の効率比較](assets/diagrams/chapter6/figure6-5-list-vs-dict-search-comparison.svg)
...
![図6-12：スタックとキューの基本概念](assets/diagrams/chapter6/figure6-12-stack-queue-basic-concepts.svg)
...
![図6-15：データ構造選択の判断フロー](assets/diagrams/chapter6/figure6-15-data-structure-selection-flow.svg)

### データ構造比較表

![図6-16：主要データ構造の性能・特徴比較](assets/diagrams/chapter6/figure6-16-data-structure-performance-comparison.svg)

### 問題別データ構造選択ガイド

![図6-17：典型的な問題パターンとデータ構造選択](assets/diagrams/chapter6/figure6-17-problem-patterns-data-structure-selection.svg)

### 実際の問題での選択例

```
【図6-18：ABC問題での実践的なデータ構造選択】
...
│ 💡 選択理由：                              │
│ • 順番に読み込む必要がある → list           │
│ • 一度だけmax()を呼ぶ → 検索速度は問題なし  │
│ • シンプルで分かりやすい                   │
└─────────────────────────────────────────────┘

🎯 ABC B問題レベル：少し複雑な処理
┌─────────────────────────────────────────────┐
│ 問題：文字列中に含まれる異なる文字の個数     │
│                                           │
│ 解法：                                     │
│ s = input()                                │
│ unique_chars = set(s)                      │
...
│                                           │
│ 💡 選択理由：                              │
│ • 検索が多数回 → O(1)の set が必須         │
│ • listだと O(N×Q)で時間制限超過の危険      │
│ • setなら O(N+Q)で高速処理                │
└─────────────────────────────────────────────┘
```

## まとめ：データ構造を適材適所で活用しよう

この章では、プログラムの効率性と可読性を大きく左右するデータ構造について詳しく学んだ。

```
【図6-19：この章で習得したデータ構造スキル】

📚 習得したデータ構造
┌─────────────────────────────────────────────┐
│ ✅ リスト（list）：順序のあるデータの管理     │
│ ✅ 辞書（dict）：高速検索とデータの対応関係   │
│ ✅ 集合（set）：重複除去と集合演算           │
│ ✅ デック（deque）：効率的なキュー・スタック  │
└─────────────────────────────────────────────┘
...
🚀 競技プログラミングでの実践力
┌─────────────────────────────────────────────┐
│ 🏆 ABC A,B問題：適切なデータ構造選択で確実に解答│
│ 🏆 ABC C問題：効率性を考慮した高速な解法     │
│ 🏆 計算量の意識：O(1)、O(N)の違いを理解     │
│ 🏆 Pythonらしいコード：内包表記やライブラリ活用│
└─────────────────────────────────────────────┘
```

**データ構造は道具箱**

データ構造は、大工さんの道具箱のようなものだ。釘を打つのにハンマーを使い、ネジを回すのにドライバーを使うように、問題に応じて最適なデータ構造を選ぶことで、効率的で美しいプログラムが書ける。
