...
│ • 最後に入れたものが最初に出てくる            │
│ • 一番上の要素のみアクセス可能               │
│ • push（追加）とpop（取り出し）の2つの操作    │
└─────────────────────────────────────────────┘

🚶 キュー（Queue）：先入れ先出し（FIFO: First In, First Out）
┌─────────────────────────────────────────────┐
│ 実世界の例：行列、パイプの中の水             │
│                                           │
│ enqueue → [人1][人2][人3] → dequeue        │
│ （後ろに並ぶ）              （前から出る）   │
│                                           │
│ 特徴：                                     │
│ • 最初に入れたものが最初に出てくる            │
│ • 前と後ろの両端からアクセス                │
│ • enqueue（追加）とdequeue（取り出し）の操作  │
└─────────────────────────────────────────────┘
```
...
```
【図6-16：主要データ構造の性能・特徴比較】

┌─────────────────────────────────────────────────────────┐
│ データ構造  │ 検索  │ 追加  │ 削除  │ 重複  │ 順序  │ 主な用途 │
├─────────────────────────────────────────────────────────┤
│ リスト      │ O(N)  │ O(1)  │ O(N)  │ 可    │ 有    │ 順序重要 │
│ list        │       │ 末尾  │ 先頭  │       │       │ 索引アクセス│
├─────────────────────────────────────────────────────────┤
│ 辞書        │ O(1)  │ O(1)  │ O(1)  │ 不可  │ 有※  │ 高速検索 │
│ dict        │       │       │       │キーで │3.7+   │ 対応関係 │
├─────────────────────────────────────────────────────────┤
│ 集合        │ O(1)  │ O(1)  │ O(1)  │ 不可  │ 無    │ 重複除去 │
│ set         │       │       │       │       │       │ 集合演算 │
├─────────────────────────────────────────────────────────┤
│ deque       │ O(N)  │ O(1)  │ O(1)  │ 可    │ 有    │ 両端操作 │
│ (キュー)    │       │両端   │両端   │       │       │ FIFO/LIFO│
└─────────────────────────────────────────────────────────┘

※ O(1)=一定時間、O(N)=要素数に比例した時間
```

### 問題別データ構造選択ガイド

```
【図6-17：典型的な問題パターンとデータ構造選択】
...
│ if 5 in numbers:  # O(N) - 遅い            │
│     print("見つかりました")                 │
│                                           │
│ ✅ 効率的な方法：                           │
│ numbers_set = {1, 5, 3, 8, 2, 9, 4}        │
│ if 5 in numbers_set:  # O(1) - 高速        │
│     print("見つかりました")                 │
│                                           │
│ 💡 選択基準：                              │
│ • 検索が頻繁 → set または dict             │
│ • 順序が重要 → list（検索は遅いが順序保持） │
└─────────────────────────────────────────────┘
...
│     print(f"実行: {current}")              │
│                                           │
│ 💡 リストだと前から取り出すのが遅い（O(N)）   │
│ 💡 dequeなら前から取り出すのも高速（O(1)）   │
└─────────────────────────────────────────────┘
