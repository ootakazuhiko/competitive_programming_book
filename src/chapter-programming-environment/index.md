...
{% capture ch3_quiz_q %}
Q1. Python を競技で選ぶ理由を2つ  
Q2. VS Code での基本操作（保存/実行/拡張インストール）の流れ  
Q3. よくあるエラーの読み方のコツ  
Q4. ライブラリのインストール時に気をつけること  
Q5. 次章の入出力学習で重要な観点は？
{% endcapture %}
{% include panel.html type="steps" title="📘 質問" content=ch3_quiz_q %}

{% capture ch3_points %}
• 可読性と標準ライブラリの豊富さ  
• エラーは落ち着いてメッセージ→該当行→原因の順に辿る  
• 開発環境は「シンプルに、使い慣れる」ことが大切
{% endcapture %}
{% include panel.html type="info" title="✅ 要点チェック" content=ch3_points %}
