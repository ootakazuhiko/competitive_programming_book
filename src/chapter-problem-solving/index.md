...
3) 本体処理を小さく区切る
{% endcapture %}
{% include panel.html type="steps" title="実装ステップ" content=impl_steps %}
...
{% include panel.html type="steps" title="最初に実装すること" content="入力・出力の雛形／main構造／必要変数の初期化" %}

<figure class="pseudocode">
<figcaption>骨組みコード</figcaption>
...
• エッジケース想定
{% endcapture %}
{% include panel.html type="steps" title="Phase 1" content=ph1 %}
...
• 実装前の詳細計画
{% endcapture %}
{% include panel.html type="steps" title="Phase 2" content=ph2 %}
...
• 可読性と保守性
{% endcapture %}
{% include panel.html type="steps" title="Phase 3" content=ph3 %}
...
• 品質保証プロセス
{% endcapture %}
{% include panel.html type="steps" title="Phase 4" content=ph4 %}
...
{% capture next %}
第8章の達成  
• 体系的問題解決／設計思考／品質保証／テスト戦略

第9章の目標  
• 実践的エラー対応／デバッグ向上／トラブル解決  
• レジリエンス強化
{% endcapture %}
{% include panel.html type="plan" title="達成と次の目標" content=next %}
...
{% capture ch8_quiz_q %}
Q1. 読解の3段階（Stage1/2/3）それぞれで何を確認する？  
Q2. サンプル検証で「通った=正しい」と言えない理由は？  
Q3. 設計段階で決めるべきことを2つ挙げる（データ構造/アルゴリズム/境界条件など）  
Q4. デバッグの第一歩として何をすべきか（再現/切り分けの観点）  
Q5. 「良いテストケース」の特徴を2つ挙げる
{% endcapture %}
{% include panel.html type="steps" title="📘 質問" content=ch8_quiz_q %}

{% capture ch8_quiz_a %}
A1. Stage1: 用語/入出力/制約、Stage2: 例で流れ確認、Stage3: 一般化と落とし穴  
A2. 反例が無数にあり、サンプルは網羅的ではないため  
A3. データ構造/アルゴリズムの選択、境界条件/例外の扱い  
A4. まず再現、最小ケースへ切り詰める、入力/状態を観察  
...
{% endcapture %}
{% include panel.html type="info" title="ℹ️ 注記（凡例）" content=ch8_note %}
